		...
		match device.class.as_str() {
			"wifi" => {
				let mut path = match &device.path {
					...
					None => {
						return Err(Error::new(
							ErrorKind::Other,
							format!("wifi device must be matched by path '{}'", name),
						))
					}
				};
				...
				if !path.starts_with("/sys/device") {
					...
					if let Ok(v) = std::fs::read_link(&path2.join("device"))
						.and_then(|v| std::path::Path::new(&path).join(v).canonicalize())
					{
				...
				if !path.starts_with("/sys/devices/") {
					return Err(Error::new(
						ErrorKind::Other,
						format!("path {} did not resolve to a sysfs device", path),
					));
				}
				path = path[13..].to_string();

				write!(
					&mut self.out_wireless,
					"
config wifi-device '{}'
...
",
				...
				let w = match device.wifi.as_ref() {
					...
					None => {
						return Err(Error::new(
							ErrorKind::Other,
							format!("missing [device.{}.wifi] section", name),
						));
					}
				};
	...
	fn wifi(&mut self, name: &str, interface: &genesis::Interface) -> Result<(), Error> {
		let device_name = match &interface.device {
			None => {
				return Err(Error::new(
					ErrorKind::Other,
					format!("wifi interface missing device {}'", name),
				))
			}
			Some(v) => match self.devices.get(v) {
				None => {
					return Err(Error::new(
						ErrorKind::Other,
						format!("undeclared interface '{}' used in device {}'", v, name),
					))
				}
				Some(_) => v.to_string(),
			},
		...
		let w = match interface.wifi.as_ref() {
			...
			None => {
				return Err(Error::new(
					ErrorKind::Other,
					format!("missing [interface.{}.wifi] section", name),
				));
			}
		};
...
",
	...
	fn create_interface(
		...
		match &interface.dhcp {
			Some(toml::value::Value::String(s)) => match s.as_str() {
				"yes" | "true" | "ipv4" => {
					i.dhcp = true;
				}
				"false" | "no" => {
	...
	fn wireguard(&mut self, name: &str, interface: &genesis::Interface) -> Result<(), Error> {
		let wg = match interface.wireguard.as_ref() {
			...
			None => {
				return Err(Error::new(
					ErrorKind::Other,
					format!("missing [interface.{}.wireguard] section", name),
				));
			}
		};
		write!(
		...
		let peer = match &wg.peers {
			...
			_ => {
				return Err(Error::new(
					ErrorKind::Other,
					format!(
						"need exactly one [interface.{}.wireguard.peers] section",
						name
					),
				));
			}
...
",
			name,
			peer.public_key,
			if peer.autoroute.clone().unwrap_or_default() {
				"1"
			} else {
				"0"
			}
		...
		if endpoint.len() != 2 {
			return Err(Error::new(
				ErrorKind::Other,
				format!("invalid wg endpoint: {}", peer.endpoint),
			));
		}
		match endpoint[1].parse::<u16>() {
			Err(e) => {
				return Err(Error::new(
					ErrorKind::Other,
					format!("invalid wg endpoint port: {}: {}", endpoint[1], e),
				));
			}
			Ok(v) => {
	...
	fn template(&mut self, _name: &str, template: &genesis::Template) -> Result<(), Error> {
		...
		let t = Handlebars::new()
			.render_template(&s, &template.vars)
	...
	pub fn commit(&self) -> Result<(), Error> {
		...
		for (path, s) in &self.tplout {
			if let Err(err) =
				std::fs::File::create(path).and_then(|mut f| f.write_all(s.as_bytes()))
			{
				log::warn!("{}: {}", path, err);
			}
		}

		Command::new("./etc/devguard/genesis/post").spawn().ok();
